...
def process_file(filepath):
    ...
    for line in lines:
        ...
        if stripped.startswith("oneset ") or stripped.startswith("oneset\t"):
            ...
            # Separate inline comments if any (simple # check)
            comment = ""
            if "#" in args_comment:
                ac_parts = args_comment.split("#", 1)
                args = ac_parts[0].strip()
                comment = "  # " + ac_parts[1].strip()
            else:
                args = args_comment
            
            # New Line Construction
            # We keep the original logic line as a comment maybe? OR just overwrite as requested.
            # User said: "hier muesste nach one_pre.py scan strRepl dann py syntax stehen"
            # IMPLIED: Replaces the custom syntax.
            
            py_code = f"{indentation}p4.oneset(\"{args}\"){comment}\n"
        ...
        # Detect 'oneget' or 'onget'
            ...
            # Logic: oneget l1 dbgg15 l2 Color
            # Need to parse this. Local vars vs Keys.
            # Assuming pairs: Var Key Var Key
            
            # Handle comments
            comment = ""
            if "#" in raw_args:
                ac_parts = raw_args.split("#", 1)
                clean_args = ac_parts[0].strip()
                comment = "  # " + ac_parts[1].strip()
            else:
                clean_args = raw_args
            ...
            try:
                ...
                py_code = f"{indentation}{lhs} = p4.oneget(\"{rhs_keys}\"){comment}\n"
                new_lines.append(py_code)
                modified = True
